...
class ParseError extends Error {
	...
	constructor(msg: string, tok?: Token, file?, parent?) {
		super();
...
class Item {
	...
	static fromRecord(record: wire.Record, comment: string) {
		const item = new Item();
		item.record = record;
	...
	static fromString(msg: string, tok: Token, file: string) {
	...
	static fromError(err: ParseError, tok: Token, file: string) {
...
class TTLState {
	...
	constructor(ttl: number, directive) {
		this.ttl = ttl || 0;
...
function* lexer(input: string, zone: boolean) {
	if (zone == null)
		zone = true;
			...
			case '\t': {
				...
				if (str.length === 0) {
					...
					if (!rrtype) {
						...
						const t = RecordType[upper];

						if ((t & 0xffff) === t && t !== RecordType.ANY) {
	...
	for (const tok of iter) {
		...
		switch (state) {
			...
			case states.EXPECT_RDATA: {
				...
				let record: wire.Record, comment: string;
				try {
...
function readRecord(_wire: typeof wire, iter, hdr, origin, file): [wire.Record, string] {
	const RD = _wire.recordsByVal[hdr.type];
	const format = schema.records[hdr.type];

	if (!RD)
		throw new ParseError('unknown rr type', null, file);

	const rr = new _wire.Record();
	const rd = new RD();
	...
	for (const [name, type] of format) {
		[field, comment] = readString(_wire, rd, type, iter, origin, file);
		rd[name] = field;
	}
...
function readUnknown(_wire: typeof wire, iter, hdr, origin, file): [wire.Record, string] {
	let RD = _wire.recordsByVal[hdr.type];

	if (!RD)
		RD = _wire.UNKNOWNRecord;
...
function readString(wire, rd, type, iter, origin, file) {
	...
	switch (type) {
		...
		case schema.U64: {
			...
			num.writeUInt32BE(hi, 0, true);
			num.writeUInt32BE(lo, 0, true);
			return [num, skip(iter, file)];
		}
...
function parseOption(wire, input) {
	...
	let code: number = +EOption[tok.string.toUpperCase()]; // TODO
	let unknown = false;
