...
export function verify(msg: Buffer, secret: Buffer, requestMAC: Buffer | null, timersOnly: boolean) {
	if (requestMAC == null)
		requestMAC = null;
...
function removeTSIG(msg: Buffer) {
	// assert(Buffer.isBuffer(msg));
	assert(msg.length >= 12);
...
function tsigData(msg: Buffer, sig: TSIGRecord, requestMAC: Buffer | null, timersOnly: boolean, offset: number) {
	// assert(Buffer.isBuffer(msg));
	assert(msg.length >= 12);
	...
	if (timersOnly) {
		...
		bw.writeU8(0);
		bw.writeU16BE(RecordType.TSIG);
		bw.writeU16BE(QuestionClass.ANY);
		bw.writeU32BE(0);
