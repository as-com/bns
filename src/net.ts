...
export class Base extends EventEmitter {
	...
	address() {
		const {address, port} = this.socket.address() as AddressInfo;
...
class Client extends Base {
	...
	async write(msg, port, host) {
		const key: string = IP.toHost(host, port);
...
export class TCPSocket extends EventEmitter {
	...
	async connect(...args) {
		...
		(this.socket.connect as any)(...args); // TODO
		this.socket.unref();
